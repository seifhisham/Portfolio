...
export default function header() {  
  ...
  return <header className="z-[999] relative">
    ...
    shadow-black/[0.03] backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] sm:w-36[rem] sm:rounded-full"
    initial={{y: -100, x: "-50%", opacity: 0}}
    animate={{y:0, x: "-50%", opacity:1}}>
    </motion.div>

    <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
      <ul className="flex w-[22rem] flex-wrap items-center justify-centerfap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial]
      sm:flex-nowrap sm:gap-5">
        {
          links.map((link) =>(
            <motion.li className="j-3/4 flex items-center justify-center relative" key={link.hash}
            initial={{y: -100, x: "-50%", opacity: 0}}
            animate={{y:0, x: "-50%", opacity:1}}>
